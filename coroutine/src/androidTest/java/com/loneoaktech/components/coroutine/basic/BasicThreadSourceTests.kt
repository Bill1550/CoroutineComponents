...
class BasicThreadSourceTests {
    ...
    fun sharedFlowTest() = runBlocking {
        ...
        val flow = callbackFlow{
            ...
            strCallback.setListener { trySend(it) }
            awaitClose { strCallback.close() }
        }.shareIn( CoroutineScope( Job() + Dispatchers.IO ), started = SharingStarted.WhileSubscribed() ).apply {
...
//    )
    ...
    fun stateFlowExample1() = runTest {
        ...
        val stateFlow = flowOfEvents.stateIn(sharingScope, started = SharingStarted.WhileSubscribed(), "initial")
